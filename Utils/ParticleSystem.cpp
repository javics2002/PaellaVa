...
void ParticleSystem::draw(SDL_Rect* cameraRect)
{
    if (_texture == nullptr)
    {
        return;
    ...
    for (int i = 0; i < _particleCount; i++)
    {
        ...
        SDL_Rect r = { int(p.posx + p.startPosX - p.size / 2) - cameraRect->x, int(p.posy + p.startPosY - p.size / 2) - cameraRect->y, int(p.size), int(p.size) };
        SDL_Color c = { Uint8(p.colorR * 255), Uint8(p.colorG * 255), Uint8(p.colorB * 255), Uint8(p.colorA * 255) };
        SDL_SetTextureColorMod(_texture, c.r, c.g, c.b);
        SDL_SetTextureAlphaMod(_texture, c.a);
        SDL_SetTextureBlendMode(_texture, SDL_BLENDMODE_BLEND);
